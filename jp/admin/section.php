...
<body class="hold-transition sidebar-mini layout-fixed">
        ...
        <div class="content-wrapper">
                            ...
                            <div class="modal-body">
                                <div class="container">
                                    <form method="POST" action="logic.php" enctype="multipart/form-data">
                                        ...
                                        <button type="button" class="btn btn-secondary" id="addFieldsBtn">Add Additional
                                            Fields</button>
                                    </form>
                                </div>
                            </div>
                ...
                $(document).ready(function() {
                    ...
                    $("#addFieldsBtn").click(function() {
                        if (fieldCounter < maxFields) {
                            ...
                            var paraField =
                                '<div class="mb-3"><label for="name" class="form-label">Para' +
                                fieldCounter +
                                '</label><input type="text" class="form-control" name="b_para' +
                                fieldCounter + '" required></div>';

                            // Add a new image field
                            var imageField =
                                '<div class="mb-3"><label for="name" class="form-label">Image' +
                                fieldCounter +
                                '</label><input type="file" class="form-control" name="b_image' +
                                fieldCounter + '" required></div>';

                            // Add a delete button with a cross symbol
                            var deleteButton =
                                '<button class="mt-3 mb-3 btn btn-sm btn-danger deleteBtn">&#10006;</button>';

                            // Append both fields and the delete button to the container
                            $("#dynamicFieldsContainer").append('<div class="fieldContainer">' +
                                deleteButton + paraField + imageField + '</div>');

                            // Increment the counter
                            fieldCounter++;
                        } else {
                            alert("Maximum field limit reached (8 fields).");
                        }
                    });

                    // Add event listener for dynamic delete buttons
                    $("#dynamicFieldsContainer").on('click', '.deleteBtn', function() {
                    ...
                    <table id="example1" class="table table-bordered table-striped">
                        <thead>
                            <tr>
                                ...
                                <th>Image2</th>
                                <?php
                // Print header for additional fields (para3 to para8 and image3 to image8)
                for ($i = 3; $i <= 8; $i++) {
                    echo "<th style='display:none;'>Para{$i}</th>";
                    echo "<th style='display:none;'>Image{$i}</th>";
                }
                ?>
                                <th>Date</th>
                                <th>Written By</th>
            ...
            if (mysqli_num_rows($result) > 0) {
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    // Loop to print additional fields (para3 to para8 and image3 to image8)
                    for ($i = 3; $i <= 8; $i++) {
                        echo "<td style='display:none;'>{$row['para'.$i]}</td>";
                        echo "<td style='display:none;'><img src='{$row['image'.$i]}' width='100' height='120' alt='blog Image'></td>";
                    }
                    ...
                    echo "<td>
                        <div class='icon-container'>
                            <a href='#' class='edit-blog' 
                                ...
                                ";
                    // Set data-paraKey and data-imageKey dynamically
                    for ($i = 3; $i <= 8; $i++) {
                        echo "data-para{$i}='{$row['para'.$i]}' data-image{$i}='{$row['image'.$i]}'";
                    }
    ...
    $(document).ready(function () {
        ...
        var additionalFieldsContainer = $('#additionalFieldsContainer');
        var existingFieldsCount = 2;
        var usedIndices = [];

        // Function to get the next unique index for new fields
        function getNextIndex() {
            var currentIndex = Math.floor(existingFieldsCount +
                additionalFieldsContainer.children().length / 2 + 1);
        ...
        // Button click to add additional fields
        $('#addAdditionalField').on('click', function () {
            // Calculate the index for the new set of fields
            var currentIndex = getNextIndex();

            // Check if index is within the limit (8 in this case)
            if (currentIndex > 8) {
                alert("Cannot add more than 8 fields.");
                return;
            }

            // Create additional field inputs
            var paraLabel =
                '<label for="name" class="form-label">Para' +
                currentIndex + '</label>';
            var paraInput =
                '<input type="text" class="form-control" name="edit_para' +
                currentIndex + '" id="edit_para' + currentIndex +
                '" required>';
            var imageLabel =
                '<label for="blog_image" class="form-label">Image ' +
                currentIndex + '</label>';
            var imageInput =
                '<input type="file" class="form-control mb-2" name="edit_image' +
                currentIndex + '" id="edit_image' + currentIndex +
                '" required>';
            var deleteButton =
                '<button class="mt-3 mb-3 btn btn-sm btn-danger deleteBtn">&#10006;</button>';

            // Create a container for the new set of fields
            var fieldContainer = $('<div class="fieldContainer"></div>');

            // Append additional fields and delete button to the container
            fieldContainer.append(deleteButton + paraLabel + paraInput +
                imageLabel + imageInput);

            // Append the container to the additional fields container
            additionalFieldsContainer.append(fieldContainer);
        });

        // Add event listener for dynamic delete buttons
        $("#additionalFieldsContainer").on('click', '.deleteBtn', function () {
            var deletedIndex = parseInt($(this).closest(
                '.fieldContainer').find(
                'label[for^="edit_para"]').text().replace(
                'Para', ''));
            usedIndices = usedIndices.filter(index => index !==
                deletedIndex);
            $(this).closest('.fieldContainer').remove();
        });

        // Existing code for other modal elements...
    });
</script>
                            ...
                            <div class="modal-footer">
                                ...
                                <button type="button" class="btn btn-secondary" id="addAdditionalField">Add
                        ...
                        <tbody>
                            ...
                            if (mysqli_num_rows($result) > 0) {
                                while ($row = mysqli_fetch_assoc($result)) {
                                    ...
                                    echo "<td style='width:200px;'><div style='height: 200px; overflow-y: scroll;'>{$row['JjobDescription']}</td>";
                                    echo "<td>{$row['JcompanyName']}</td>";
                                    echo "<td>{$row['Jtagline']}</td>";
                                    echo "<td style='width:200px;'><div style='height: 200px; overflow-y: scroll;'>{$row['JcompanyDescription']}</td>";
                                    echo "<td>{$row['Jwebsite']}</td>";
                                    echo "<td>{$row['JfacebookUsername']}</td>";
                                    echo "<td>{$row['JtwitterUsername']}</td>";
                                    echo "<td>{$row['JlinkedinUsername']}</td>";
                                    echo "<td><img src='../{$row['jobImage']}' width='100' height='120' alt='job image'></td>";


                                    echo "<td><img src='../{$row['Jlogo']}' width='100' height='120' alt='logo image'></td>";
    ...
    $(document).ready(function() {
        $('.edit-blog').on('click', function() {
            ...
            for (var i = 3; i <= 8; i++) {
                ...
                // Check if data exists for additional fields
                if ($(this).data(paraKey) && $(this).data(imageKey)) {
                    var paraValue = $(this).data(paraKey);
                    var imageValue = $(this).data(imageKey);

                    // Create new input fields for additional fields
                    var paraField = '<div class="mb-3"><label for="edit_' + paraKey +
                        '" class="form-label">' + paraKey +
                        '</label><input type="text" class="form-control" name="edit_' + paraKey +
                        '" id="edit_' + paraKey + '" value="' + paraValue + '" required></div>';
                    var imageField = '<div class="mb-3"><label for="edit_' + imageKey +
                        '" class="form-label">' + imageKey +
                        '</label><input type="file" class="form-control" name="edit_' + imageKey +
                        '" id="edit_' + imageKey +
                        '" required><img width="100" height="120" id="edit_' + imageKey +
                        'Preview" src="' + imageValue + '" alt=""></div>';

                    // Append new fields to the container
                    $('#additionalFieldsContainer').append(paraField + imageField);
                }
            }
