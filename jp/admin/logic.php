...
if (isset($_POST["submit"])) {
    ...
    $uploadedImage3 = isset($_FILES['b_image3']) ? handleImageUpload($_FILES['b_image3'], 3) : null;
    $uploadedImage4 = isset($_FILES['b_image4']) ? handleImageUpload($_FILES['b_image4'], 4) : null;
    $uploadedImage5 = isset($_FILES['b_image5']) ? handleImageUpload($_FILES['b_image5'], 5) : null;
    $uploadedImage6 = isset($_FILES['b_image6']) ? handleImageUpload($_FILES['b_image6'], 6) : null;
    $uploadedImage7 = isset($_FILES['b_image7']) ? handleImageUpload($_FILES['b_image7'], 7) : null;
    $uploadedImage8 = isset($_FILES['b_image8']) ? handleImageUpload($_FILES['b_image8'], 8) : null;

    // Check and set para variables
    $b_para1 = isset($_POST['b_para1']) ? mysqli_real_escape_string($config, $_POST['b_para1']) : null;
    $b_para2 = isset($_POST['b_para2']) ? mysqli_real_escape_string($config, $_POST['b_para2']) : null;
    $b_para3 = isset($_POST['b_para3']) ? mysqli_real_escape_string($config, $_POST['b_para3']) : null;
    $b_para4 = isset($_POST['b_para4']) ? mysqli_real_escape_string($config, $_POST['b_para4']) : null;
    $b_para5 = isset($_POST['b_para5']) ? mysqli_real_escape_string($config, $_POST['b_para5']) : null;
    $b_para6 = isset($_POST['b_para6']) ? mysqli_real_escape_string($config, $_POST['b_para6']) : null;
    $b_para7 = isset($_POST['b_para7']) ? mysqli_real_escape_string($config, $_POST['b_para7']) : null;
    $b_para8 = isset($_POST['b_para8']) ? mysqli_real_escape_string($config, $_POST['b_para8']) : null;
    ...
    $insertQuery = "INSERT INTO blog (title, frontline, titleimage, para1, image1, para2, image2, para3, image3, para4, image4, para5, image5, para6, image6, para7, image7, para8, image8, date, writtenby) 
                   VALUES ('$bTitle', '$b_frontline', '$uploadedImageTitle', 
                           '$b_para1', '$uploadedImage1', 
                           '$b_para2', '$uploadedImage2', 
                           '$b_para3', '$uploadedImage3', 
                           '$b_para4', '$uploadedImage4', 
                           '$b_para5', '$uploadedImage5', 
                           '$b_para6', '$uploadedImage6', 
                           '$b_para7', '$uploadedImage7', 
                           '$b_para8', '$uploadedImage8', 
                           '$b_date', '$b_written')";

    if (mysqli_query($config, $insertQuery)) {
        echo "<script>alert('Blog inserted successfully.');window.location.href='section.php?section=blog';</script>";
    } else {
...
if (isset($_POST["id"])) {
    ...
    // Loop to handle additional fields (para3 to para8 and image3 to image8)
    for ($i = 3; $i <= 8; $i++) {
        $paraKey = "edit_para" . $i;
        $imageKey = "edit_image" . $i;

        if (isset($_POST[$paraKey])) {
            $paraValue = $_POST[$paraKey];
            $updateQuery .= ", para{$i}='$paraValue'";
        }

        if (isset($_FILES[$imageKey]) && $_FILES[$imageKey]['error'] == 0) {
            $targetDir = "upload_images/";
            $targetFileImage = $targetDir . basename($_FILES[$imageKey]["name"]);

            if (move_uploaded_file($_FILES[$imageKey]["tmp_name"], $targetFileImage)) {
                $updateQuery .= ", image{$i}='$targetFileImage'";
            } else {
                $response['status'] = 'error';
                $response['message'] = "Error uploading {$imageKey}";
                echo json_encode($response);
                exit;
            }
        }
